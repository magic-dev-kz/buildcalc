...
- **Performance: deferred non-critical init**: History, PWA install, material lookup, print report, share project, and SW registration wrapped in `requestIdleCallback` (with `setTimeout` fallback) so main UI renders before secondary features load
- SW cache bumped to `buildcalc-v26.0`
...
- SW cache bumped to `buildcalc-v25.0`
